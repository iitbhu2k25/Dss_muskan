...
export const CategoryProvider = ({ children }: CategoryProviderProps) => {
  ...
  // Get selected condition categories with weights
  const getSelectedConditionCategories = (): SelectRasterLayer[] => {
    const selectedCategories = conditionCategories
      .filter(category => selectedCondition.some(item => item.file_name === category.file_name))
      .map(category => {
        // Find the custom Influence if it exists
        const customInfluence = selectedCondition.find(
          item => item.file_name === category.file_name
        )?.Influence;
        
        return {
          file_name: category.file_name,
          Influence: customInfluence || category.weight.toString()
        };
      });
  ...
  // Get selected constraint categories with weights
  const getSelectedConstraintCategories = (): SelectRasterLayer[] => {
    const selectedCategories = constraintCategories
      .filter(category => selectedConstraint.some(item => item.file_name === category.file_name))
      .map(category => {
        // Find the custom Influence if it exists
        const customInfluence = selectedConstraint.find(
          item => item.file_name === category.file_name
        )?.Influence;
  ...
  const toggleConditionCategory = (file_name: string): void => {
    setSelectedCondition(prev => {
      ...
      if (isSelected) {
        ...
        if (category) {
          newSelection = [...prev, { file_name, Influence: category.weight.toString() }];
        } else {
          newSelection = prev;
        }
      }
  ...
  const updateConditionCategoryInfluence = (file_name: string, Influence: number): void => {
    ...
    setSelectedCondition(prev => {
      ...
      if (categoryIndex !== -1) {
        ...
        const category = conditionCategories.find(cat => cat.file_name === file_name);
        if (category) {
          updatedCategories = [...prev, { file_name, Influence: clampedInfluence.toString() }];
        } else {
          updatedCategories = prev;
        }
      }
  ...
  const selectAllConditionCategories = (): void => {
    ...
    setSelectedCondition(calculateWeights(allCategories));
  };
  ...
  const selectAllConstraintCategories = (): void => {
    ...
    // Calculate weights and update state
    setSelectedConstraint(calculateWeights(allCategories));
  };
  ...
  const contextValue: CategoryContextType = {
    ...
    selectedCondition: getSelectedConditionCategories(),
    selectedConstraint: getSelectedConstraintCategories(),
    selectedCategory,
    setSelectedCategory,
