...
class Geoserver:
    ...
    def publish_raster(self,workspace_name, store_name, raster_path):
        try:
            ...
            check_url = f"{self.geoserver_url}/rest/workspaces/{workspace_name}/coveragestores/{store_name}"    
            check_response = requests.get(
                check_url,
                auth=HTTPBasicAuth(self.username, self.password)
            )
            ...
            if check_response.status_code != 200:
                print(f"Coverage store '{store_name}' does not exist in workspace '{workspace_name}'. Creating it...")
            ...
            # Try to delete existing layer if it exists
            delete_url = f"{self.geoserver_url}/rest/workspaces/{workspace_name}/coveragestores/{store_name}/coverages/{layer_name}?recurse=true"
            delete_response = requests.delete(
                delete_url,
                auth=HTTPBasicAuth(self.username, self.password)
            )
            if delete_response.status_code == 200:
                print(f"Existing raster layer '{layer_name}' deleted for overwrite")

            upload_url = f"{self.geoserver_url}/rest/workspaces/{workspace_name}/coveragestores/{store_name}/{api_extension}?configure=all"
            ...
            if response.status_code in (200, 201):
                print(f"Raster file uploaded and published as layer '{layer_name}'")
                
                # After upload, explicitly configure the coverage with WMS enabled
                ...
                if configure_response.status_code in (200, 201):
                    print(f"Raster layer '{layer_name}' configured successfully with WMS enabled")
                else:
                    print(f"Warning: Layer uploaded but configuration may not be complete: {configure_response.status_code}")
                
                # Verify WMS is enabled for the layer
                ...
                if verify_response.status_code == 200:
                    layer_info = verify_response.json()
